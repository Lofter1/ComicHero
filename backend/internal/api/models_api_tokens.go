...
// Scopes that can be granted to an API token. These intentionally cover
// only the read/write surface an external service (a reading server,
// collection manager, etc.) needs to integrate with a user's reading
// activity - not full account access.
const (
	apiTokenScopeReadingRead  = "reading:read"
	apiTokenScopeReadingWrite = "reading:write"
)

var validAPITokenScopes = map[string]bool{
	apiTokenScopeReadingRead:  true,
	apiTokenScopeReadingWrite: true,
}
...
type APIToken struct {
	...
	Scopes      []string `json:"scopes"      doc:"Granted permission scopes." example:"reading:read"`
...
type CreateAPITokenPayload struct {
	...
	Scopes    []string `json:"scopes"    doc:"Permission scopes to grant. Combine reading:read and reading:write." example:"reading:read"`

...
<template>
  ...
  >
    ...
    >
      ...
      <form
        class="collection-search-form my-4 mx-0 grid [grid-template-columns:minmax(0,_1fr)_max-content] gap-2.5 [&_input]:w-full [&_input]:min-w-0 [&_input]:min-h-10.5 [&_input]:border [&_input]:border-line-strong [&_input]:rounded [&_input]:bg-surface [&_input]:text-control [&_input]:py-2.25 [&_input]:px-3 [&_input:focus]:outline-3 [&_input:focus]:outline-focus down-compact:grid-cols-1"
      ...
      >
        ...
        >
          ...
          >
            <span
              class="collection-dialog-avatar grid place-items-center w-9.5 h-9.5 overflow-hidden border border-line-strong rounded-md bg-surface-muted text-primary-strong font-black [&_img]:w-full [&_img]:h-full [&_img]:object-cover"
              aria-hidden="true"
            >

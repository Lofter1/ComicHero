...
- `open` allows anyone who can reach the server to register with a name and password, without an invite token.

Open registration is intended only for instances you deliberately expose for self-service signup. ComicHero uses a shared-library model: comics, arcs, series, characters, and reading orders are not isolated per account. A new account can read and write the shared library data according to the app's normal user capabilities, so leave registration on `invite_only` unless you understand and accept that exposure. Admins can change the mode and remove unwanted accounts from the user management screen.
